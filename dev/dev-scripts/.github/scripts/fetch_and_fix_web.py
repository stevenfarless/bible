...
Step 1 — Fetch WEB:
  ...
  translations/WEB/{Book}.json + translations/WEB/meta.json to
  TARGET_BRANCH in a single commit.
...
def parse_usfx(xml_bytes):
    ...
    IGNORE = {"note","f","x","fig","milestone","optionalLineBreak","table","tr","th","thr","tc","tcr"}

    for event, elem in ET.iterparse(io.BytesIO(xml_bytes), events=("start", "end")):
        tag = elem.tag.split("}")[-1] if "}" in elem.tag else elem.tag
        if event == "start":
            if tag == "book":
                flush(); current_book = elem.get("id", "").upper(); current_chapter = current_verse = None; verse_parts = []
            elif tag == "c":
                flush(); current_chapter = elem.get("id"); current_verse = None; verse_parts = []
            elif tag == "v":
                flush(); current_verse = elem.get("id"); verse_parts = []
                if elem.text: verse_parts.append(elem.text)
            elif tag == "ve":
                flush(); current_verse = None
            elif current_verse and tag not in IGNORE:
                if elem.text: verse_parts.append(elem.text)
        elif event == "end" and tag not in ("v","ve","c","book") and current_verse:
            if elem.tail: verse_parts.append(elem.tail)
...
def fetch_and_push_web(owner, repo):
    ...
    print(f"  {len(books)} books found: {len(known)} recognized, {len(skipped)} skipped: {skipped}")
    ...
    for book_id, chapters in books.items():
        ...
        file_list.append((f"{TARGET_DIR}/{book_name}.json", json.dumps(out, ensure_ascii=False, separators=(",", ":"))))
        meta_books.append({"name": book_name, "testament": testament_for_id(book_id), "chapters": len(chapters)})
    ...
    file_list.append((f"{TARGET_DIR}/meta.json", json.dumps({"info": WEB_META_INFO, "books": meta_books}, indent=2, ensure_ascii=False)))

    print(f"  {len(file_list)} files to commit")
    tip_sha, base_tree_sha = get_branch_tip(owner, repo)
    print(f"  Branch {BRANCH} @ {tip_sha}")
    commit_files(owner, repo, tip_sha, base_tree_sha, file_list,
...
def fix_meta(owner, repo):
    ...
    for tid in sorted(trans_dirs):
        ...
        for book_name, blob_sha in book_files.items():
            ...
            if book_name in existing:
                ...
                entry = {"name": book_name, "testament": testament_for_name(book_name), "chapters": chapter_count}
    ...
    commit_files(owner, repo, tip_sha, base_tree_sha, new_blobs,
