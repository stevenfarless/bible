...
SHORTCUTS_SECTION_START = '\t\t\t\t\t\t\t<div class="sub-accordion-section" data-section="shortcuts">'
HELP_SHORTCUTS_MARKER = '/* Help & Shortcuts */'

SOURCE_SECTION_RE = re.compile(
    r'\n\t{7}<div class="source-code-section" aria-labelledby="sourceCodeTitle">.*?\n\t{7}</div>\n',
    re.DOTALL,
)

SOURCE_ACCORDION_RE = re.compile(
    r'\n\t{7}<div class="sub-accordion-section" data-section="source-code">.*?\n\t{7}</div>',
    re.DOTALL,
)

SOURCE_CSS_RE = re.compile(
    r'\n/\* Source code links \*/\n(?:\.source-code-[^{]+\{.*?\}\n\n?)+',
    re.DOTALL,
)
...
def update_index():
    ...
    html = SOURCE_SECTION_RE.sub('\n', html)
    html = SOURCE_ACCORDION_RE.sub('', html)

    if SHORTCUTS_SECTION_START not in html:
        raise SystemExit('Could not find Keyboard Shortcuts section in index.html.')

    html = html.replace(SHORTCUTS_SECTION_START, SOURCE_CODE_ACCORDION_HTML + '\n' + SHORTCUTS_SECTION_START, 1)
...
def main():
    ...
    print('Source code accordion applied.' if changed else 'Source code accordion already present.')
