...
> **This is the `refactor/split-app-js` branch** — branched from `search`. Purpose: break `app.js` into smaller, focused modules.
>
> ```
> main
> └── dev
>     └── search
>         └── refactor/split-app-js  ← YOU ARE HERE
> ```
>
> **Merge order (bottom-up):**
> 1. `refactor/split-app-js` → `search`  ← this branch merges here first
> 2. `search` → `dev`
> 3. `dev` → `main`
>
> Do not merge this branch into `dev` or `main` directly. It must go into `search` first so the search feature and the refactor are integrated and tested together before moving up the chain.
