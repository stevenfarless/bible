...
export function attachEventListeners(app) {
    ...
    document.getElementById('megasearchToggle')?.addEventListener('change', (e) => {
        ...
        if (e.target.checked) {
            if (query.trim().length >= 3 && app.currentSearchResults?.length > 0) runMegasearch(app, query);
        } else {
            if (query.trim().length > 0) performKeywordSearch(app, query);
        }
    });
    ...
    [app.bookModal, app.chapterModal, app.verseModal, app.settingsModal, app.loginModal, app.signupModal, app.userMenuModal, app.referencesModal, app.translationModal, app.deuterocanonInfoModal].forEach(modal => {
        if (!modal) return;
    ...
    if (readingFontSelector) {
        readingFontSelector.addEventListener('change', async () => {
            ...
            if (app.currentUser) {
                await app.database.ref(`users/${app.currentUser.uid}/settings/readingFont`).set(font);
            }
        });
    }
    ...
    document.getElementById('showSignupLink')?.addEventListener('click', (e) => { e.preventDefault(); app.closeModal(app.loginModal); app.openModal(app.signupModal); });
    document.getElementById('showLoginLink')?.addEventListener('click', (e) => { e.preventDefault(); app.closeModal(app.signupModal); app.openModal(app.loginModal); });
