...
class BibleApp {
    ...
    async handlePassageReference(reference) {
        ...
        if (data && data.passages && data.passages.length > 0) {
            ...
            const preview = this.stripHTMLExcludingVerseNumbers(data.passages[0]).substring(0, 200);
    ...
    // Like stripHTML but removes .verse-num elements first so verse numbers
    // don't appear in plain-text previews (e.g. reference search results).
    stripHTMLExcludingVerseNumbers(html) {
        const tmp = document.createElement('div');
        tmp.innerHTML = html;
        tmp.querySelectorAll('.verse-num').forEach((el) => el.remove());
