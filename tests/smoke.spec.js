...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    ...
    try {
      const syncPromptTest = new URLSearchParams(location.search).has(
        "syncPromptTest",
      );
...
async function openSettingsSection(page, sectionDataValue) {
  ...
  const section = page.locator(
    `.accordion-section[data-section="${sectionDataValue}"]`,
  );
  const isActive = await section.evaluate((el) =>
...
async function switchTranslation(page, translationId) {
  ...
  await page
    ...
    .filter({
      has: page.locator(".translation-modal-item__name", {
        hasText: translationId,
      }),
    })
    .click();
...
test("reference picker: book, chapter, verse flow supports verse and Go", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
  ...
  await page
    .locator('.book-category[data-testament="New Testament"] .book-item', {
      hasText: "John",
    })
    .first()
    .click();
...
test("book selector: testament filters toggle sections from canon data", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
  ...
  const oldSection = page.locator(
    '.book-category[data-testament="Old Testament"]',
  );
  const newSection = page.locator(
    '.book-category[data-testament="New Testament"]',
  );
  const deuterocanonSection = page.locator(
    '.book-category[data-testament="Deuterocanon"]',
  );
  ...
  await expect(
    page.locator("#referencePickerView .book-item.picker-item--active"),
  ).toHaveCount(0);
  await expect(
    page.locator(
      "#referencePickerModal .modal-content > .book-testament-filters",
    ),
  ).toHaveCount(1);
  await expect(
    page.locator("#referencePickerModal .modal-body > .book-testament-filters"),
  ).toHaveCount(0);
...
test("chapter navigation: selecting a chapter opens verse picker and Go keeps chapter", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("verse navigation: selecting a verse closes the reference picker", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("translation: switching translation reloads passage in new translation", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("search: mobile results keep the final book visible after scroll", async ({
  page,
}) => {
  ...
  await page
    .locator('.search-expand-collapse-btn[data-action="collapse"]')
    .click();
  await page
    .locator('.search-group-heading[data-testament="Old Testament"]')
    .click();
  await page
    .locator('.search-group-heading[data-testament="New Testament"]')
    .click();
  ...
  expect(layout.finalBookBottom).toBeLessThanOrEqual(
    layout.containerBottom + 1,
  );
  expect(layout.resultsClientHeight).toBeGreaterThan(0);
  expect(layout.resultsScrollHeight).toBeGreaterThan(
    layout.resultsClientHeight,
  );
...
test("search: reference query navigates to correct passage", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("search: closing search clears input and hides panel", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("passage cache: navigating back to a visited passage writes cache", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("settings: toggling verse numbers checkbox changes its state", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("settings: verse-by-verse mode toggles passage layout class", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
  ...
  const initial = await checkbox.isChecked();
  await checkbox.click();
  await expect(page.locator("#passageText")).toHaveClass(
    initial ? /^(?!.*verse-by-verse)/ : /verse-by-verse/,
  );
});
...
test("settings: font size buttons update passage font size", async ({
  ...
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("fontSize")))
    .toBe("23");
...
test("settings: color theme selector applies theme to body", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("theme switch: segmented appearance control changes body class", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
  ...
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("lightMode")))
    .toBe("light");
...
test("dynamic book picker: translation without meta.json uses 66-book fallback", async ({
  page,
}) => {
  await page.route("**/translations/KJV/meta.json", (route) =>
  ...
  await page.locator("#bookSelector").click();
  await expect(
    page.locator('.book-category[data-testament="Old Testament"] .book-item'),
  ).toHaveCount(39);
  await expect(
    page.locator('.book-category[data-testament="New Testament"] .book-item'),
  ).toHaveCount(27);
});

test("dynamic book picker: switching to BSB fires _rebuildBibleBooks", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("dynamic book picker: book modal re-renders while open on translation switch", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("dynamic book picker: book not in canon redirects to Genesis 1", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
...
test("dynamic book picker: meta.json network error falls back gracefully", async ({
  page,
}) => {
  const errors = collectPageErrors(page);
  ...
  await page.locator("#bookSelector").click();
  await expect(page.locator("#referencePickerModal")).toBeVisible();
  const otBooks = page.locator(
    '.book-category[data-testament="Old Testament"] .book-item',
  );
  const ntBooks = page.locator(
    '.book-category[data-testament="New Testament"] .book-item',
  );
  expect(await otBooks.count()).toBeGreaterThan(0);
  expect(await ntBooks.count()).toBeGreaterThan(0);
});

test("auth: unauthenticated user button opens login modal", async ({
  page,
}) => {
  await page.goto("/");
  await waitForApp(page);
...
test("auth: signup with short password shows validation toast", async ({
  page,
}) => {
  await page.goto("/");
  await waitForApp(page);
...
test("sync prompt: remains hidden during signed-out startup", async ({
  page,
}) => {
  ...
  expect(await page.evaluate(() => window._bibleApp.currentUser)).toBeNull();

  await expect(page.locator("#syncPrompt")).toBeHidden();
  await expect(page.locator("#settingsModal")).not.toHaveClass(/active/);
});

test("sync prompt: appears when a signed-out user opens settings", async ({
  page,
}) => {
  await page.goto("/?syncPromptTest=1");
  await waitForApp(page);
  ...
  expect(
    await page
      .locator("#settingsModal .modal-body")
      .evaluate((element) => element.scrollTop),
  ).toBe(0);
});

test("sync prompt: remains hidden when a signed-in user opens settings", async ({
  page,
}) => {
  await page.goto("/?syncPromptTest=1");
  await waitForApp(page);
...
test("sync prompt: remains hidden before authentication resolves", async ({
  page,
}) => {
  await page.goto("/?syncPromptTest=1");
  await waitForApp(page);
...
test("sync prompt: dismissal persists across settings openings", async ({
  page,
}) => {
  await page.goto("/?syncPromptTest=1");
  await waitForApp(page);
  ...
  await expect
    .poll(() =>
      page.evaluate(() => localStorage.getItem("syncPromptDismissedV1")),
    )
    .toBe("1");
...
test("sync prompt: closing settings does not persist dismissal", async ({
  page,
}) => {
  await page.goto("/?syncPromptTest=1");
  await waitForApp(page);
  ...
  expect(
    await page.evaluate(() => localStorage.getItem("syncPromptDismissedV1")),
  ).toBeNull();
...
test("sync prompt: sign in opens login without persisting dismissal", async ({
  page,
}) => {
  await page.goto("/?syncPromptTest=1");
  await waitForApp(page);
...
test("sync prompt: stays inside settings on desktop and mobile", async ({
  page,
}) => {
...
test("translation sync: offers KJV and BSB without starting a download", async ({
  page,
}) => {
  await page.goto("/");
  await waitForApp(page);
...
test("translation sync: BSB fallback keeps the synced preference", async ({
  page,
}) => {
  await page.goto("/");
  await waitForApp(page);
...
test("modal focus: closing login does not leave focus in an aria-hidden modal", async ({
  page,
}) => {
  await page.goto("/");
  await waitForApp(page);
...
test("auth restoration: delayed remote position cannot overwrite local navigation", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
  ...
  expect(result.events).toContain(
    "auth restoration: skipped remote position after local interaction during remote read",
  );
});

test("cold startup: passage space stays reserved until the first passage renders", async ({
  page,
}) => {
  let releasePassage;
  const passageGate = new Promise((resolve) => {
  ...
  const loadingState = await page.evaluate(() => ({
    ...
    minHeight: parseFloat(
      getComputedStyle(document.getElementById("passageText")).minHeight,
    ),
    placeholder: Boolean(
      document.querySelector(".passage-loading-placeholder"),
    ),
  }));
  ...
  await expect
    .poll(() =>
      page.evaluate(() => document.body.classList.contains("passage-ready")),
    )
    .toBe(true);
  await expect(page.locator(".passage-loading-placeholder")).toHaveCount(0);
});

test("about: GitHub release checks wait for browser idle time after settings opens", async ({
  page,
}) => {
...
test("about: marked loads only after delayed release metadata", async ({
  page,
}) => {
...
test("startup: visible monospace UI avoids loading iA Writer Mono", async ({
  page,
}) => {
  ...
  const families = await page.evaluate(() => ({
    build: getComputedStyle(document.getElementById("build-info")).fontFamily,
    chapter: getComputedStyle(document.getElementById("currentChapter"))
      .fontFamily,
    verse: getComputedStyle(document.getElementById("currentVerse")).fontFamily,
    translation: getComputedStyle(document.getElementById("currentTranslation"))
      .fontFamily,
  }));
...
test("accessibility: copyright text meets WCAG AA contrast", async ({
  page,
}) => {
  await page.goto("/");
  await waitForPassage(page);
  ...
  const ratio = await page.locator("#copyright").evaluate((element) => {
    ...
    const background = parseRgb(getComputedStyle(container).backgroundColor);
