...
export function initializeState() {
  return {
    ...
    selectedVerse: null,
    currentMaxVerse: null, // NEW: actual last verse for current chapter when known
...
export function applyVerseGlow(app) {
  ...
  if (lineSpans.length > 0) {
    ...
    const verseLines = [];
    for (const span of lineSpans) {
      if (span.id === verseId) verseLines.push(span);
    }
    ...
    // Hide the original lines AND their tags
